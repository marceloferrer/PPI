### Automatically execute buy/sell operation when detecting discrepancies in the bond market
...
tickers = ["AE38", "AL29", "AL35", "AL41","GD30","GD38","GD29","GD41"]
puntas = {
    "PESOS-INMEDIATA": 0,
    "AE38-INMEDIATA": 0,
    "AE38-A-48HS": 0,
...
def operar(ticker, buy_price, sell_price):
    try:
        if puntas["EJECUTANDOSE"] == 0:
            ...
            buy = ppi.orders.confirm(OrderConfirm("NRO_COMITENTE", 20000, buy_price, ticker, "BONOS", "Dinero",
                ...
                try:
                    ...
                    if detail['status'] == 'EJECUTADA':
                        ...
                        confirmation = ppi.orders.confirm(
                            OrderConfirm("NRO_COMITENTE", buy['quantity'], sell_price, ticker, "BONOS", "Papeles",
                        ...
                        if x == 44:
                            ...
                            cancel = ppi.orders.cancel_order(Order(buy['id'], "XXX", None))
                            print(cancel)
...
def evaluarPuntas(ticker):
    ...
    current_book_INM = ppi.marketdata.book(SearchMarketData(ticker, "BONOS", "INMEDIATA"))
    print(current_book_INM)
    if len(current_book_INM['offers']) > 0:
        buy_price = current_book_INM['offers'][0]['price']
        buy_tot = current_book_INM['offers'][0]['quantity'] * buy_price / 100
    else:
        buy_price = 0
        buy_tot = 0

    current_book_48 = ppi.marketdata.book(SearchMarketData(ticker, "BONOS", "A-48HS"))
    print(current_book_48)
    if len(current_book_48['bids']) > 0:
        sell_price = current_book_48['bids'][0]['price']
        sell_tot = current_book_48['bids'][0]['quantity'] * sell_price / 100
    else:
        sell_price = 0
        sell_tot = 0
        ...
        def onconnect_marketdata():
            try:
                ...
                for ticker in tickers:
                    ppi.realtime.subscribe_to_element(Instrument(ticker, "BONOS", "A-48HS"))
                    ppi.realtime.subscribe_to_element(Instrument(ticker, "BONOS", "INMEDIATA"))
            except Exception as error:
                traceback.print_exc()
