...
def main():
    try:
        ...
        nombre = input("Ingrese nombre de bono a buscar\n")
        ...
        print(f"Realizando busqueda de {nombre} desde el {previous.strftime('%Y-%m-%d')}")
        market_data = ppi.marketdata.search(SearchDateMarketData(nombre, "Bonos", "A-48HS",
                                                                 previous.strftime('%Y-%m-%d'),
                                                                 now.strftime('%Y-%m-%d')))

        df_market_data = api_to_dataframe(market_data)

        print("\nGenerando medias")
        df = df_market_data[["price"]].copy()
        ...
        df.to_excel(f"{nombre} ({dias}) {now.strftime('%Y-%m-%d %H_%M')}.xlsx")
